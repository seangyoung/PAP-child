...
**Note that this is different from what was done in PAP-VA, where the neighbourhoods shapefile was imported from their data portal  (using `read_sf("https://data.richmondgov.com/resource/7juf-nwis.geojson")`).** 

The `class`, `dim` and `names` are used to understand what is inside this data frame. 
...
nbr %>%
  ...
  labs(title = "LR Shapefile") + plotTheme()+
...
**Note** One need the Google API to be able to pull the map, which is not shown here. Also, PAP-VA used `get_map` function, which is a little different from the `get_googlemap` function here. It seems `get_map` does not work with this data. 
...
This chunk will create a fishnet of size `r fishnet_grid_dim` from the `nbr` shapefile that was read earlier, and aggregate child maltreatment by the grid-cells (`net_agg`). Next, the function `st_intersects()` returns the indices for intersecting cells and the `net_agg` data frame. The functions `st_join()` performs spatial left or inner join and `nb2listw()` attaches spatial weights to an `nb` list. 


**Note that the `st_crs` step here is a 'hackish' way of coercing the two objects to have the same CRS, and it does not reproject the data, but it should not be necessary unless there coordinate reference systems don't match. One way of checking this is to perform `cps_dissolve %>% st_crs() == nbr %>% st_crs()`.**
...
Now, for the current project, we will use ACS data extracted and re-coded separately, depending on which features are more important in this context. **I am still running these lines as the remainder of this code critically depends on the data-sets created at this step. It should be possible to "replace" these decennial or ACS data-sets at a later stage.**
...
**Note: Once again, the setting the "prjstring" using `st_crs()` is not a good idea, and it does not work here.**

```{r, t_intersection, eval = T, echo = T}
st_crs(littlerock_tract) <- "+proj=lcc +lat_1=34.76666666666667 +lat_2=33.3 +lat_0=32.66666666666666 +lon_0=-92 +x_0=400000 +y_0=400000
+ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"

(net_blocks_intersect <- st_intersection(littlerock_tract, net_littlerock))
```

**A possible reason could be that the `net_littlerock' data, created from the `nbr` data, is being projected in a wrong location.**
...
(p2 <- net_littlerock %>%
  ...
  geom_sf(color = NA) + 
  scale_fill_viridis_c(option = "magma") + 
  labs(title = "Fishnet")+plotTheme())
```

## Next steps 


```{r, t_intersection_2, eval = F, echo = T}
